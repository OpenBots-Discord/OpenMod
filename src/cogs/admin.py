...
class Admin(commands.Cog, name='Admin'):
    ...
    async def invite(self, ctx: SlashContext):
        ...
        embed = discord.Embed(title=STRINGS['general']['botinvitetitle'], colour=discord.Colour(0xff6900),url=f"https://discord.com/api/oauth2/authorize?client_id={bot.user.id}&permissions=204859462&scope=applications.commands%20bot",description=STRINGS['general']['botinvitedesc'])
        embed.set_author(name=STRINGS['general']['botinvitedescd'],url=f"https://discord.com/oauth2/authorize?client_id={bot.user.id}&scope=bot&permissions=204557314")
        embed.add_field(name=STRINGS['general']['botupsdc'], value=f"https://bots.server-discord.com/{bot.user.id}",inline=True)
        embed.add_field(name=STRINGS['general']['botuptopgg'], value=f"https://top.gg/bot/{bot.user.id}",inline=True)
        embed.add_field(name=STRINGS['general']['botupbod'], value=f"https://bots.ondiscord.xyz/bots/{bot.user.id}",inline=True)
        embed.set_footer(text=self.bot.user.name, icon_url=self.bot.user.avatar_url)
        await ctx.send(embed=embed)
